...
local dprint = DelranUtils.GetDebugPrint("[DELRAN'S CLICK TO WEAR]");

--[[ function ISDropItemActionWithOffset(player, item, square, x, y, z)
  local o = ISDropWorldItemAction:new(player, item, square, x, y, z);
  o.x = x;
  o.y = y;
  o.z = z;
  return o;
...
function ISDropItemAction:perform()
  ...
  self.character:getCurrentSquare():AddWorldInventoryItem(self.item, self.x, self.y, self.z);
...
function MoveToAndWear(player, worldItem)
  ...
  local offsetX = worldItem:getWorldPosX() - square:getX();
  local offsetY = worldItem:getWorldPosY() - square:getY();
  --local rotation = wearableItem;
  local offsetZ = worldItem:getWorldPosZ();
  ...
  --[[   for i = 0, square:getObjects():size() - 1 do
    ...
    if object:getProperties():getSurface() and object:getProperties():getSurface() > 0 then
      ...
      offsetZ = (object:getProperties():getSurface() / 192) * 2;
      break;
    end
  end
 ]]
  ...
  -- Wear the item.
  ISTimedActionQueue.add(ISWearClothing:new(player, wearableItem, time));
  ...
  if replacingItems then
    ...
    ISTimedActionQueue.add(ISDropWorldItemAction:new(player, item, square, offsetX, offsetY, offsetZ, 0, false));
  end
  --[[
  local i = 0;
  OnTick = function()
